...
trait RussianCurrencySupport
{
     ...
     * */
    protected function get_amount_in_rub($amount){
        if ($this->getCurrency() === 'RUB'){
            return $amount;
        }
        $payable_amount = $this->make_amount_in_rub($amount, $this->getCurrency());
        if ($payable_amount < 1) {
     ...
     * */
    private function make_amount_in_rub($amount, $currency)
    {
        $output = 0;
